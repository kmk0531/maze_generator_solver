...
import cv2
import random
from DFS_backtracking_ver2 import solve_maze as bfs_solve
...
class App:
    ...
    def run(self):
        ...
        print("        [알고리즘 선택] 1=BFS (Teammate) / 2=BFS (User) / 3=DFS (User)")
        paused = False
        ...
        while True:
            ...
            if self.state == "GENERATE":
                ...
                if self.solve_iter is None:
                    if self.algo_mode == "BFS (Teammate)":
                        self.solve_iter = bfs_solve(ROWS, COLS, self.rn.grid, self.rn.start, self.rn.end)
                    elif self.algo_mode == "BFS (User)":
                        self.solve_iter = solve_maze_user_bfs(ROWS, COLS, self.rn.grid, self.rn.start, self.rn.end)
                    elif self.algo_mode == "DFS (User)":
                        self.solve_iter = solve_maze_user_dfs(ROWS, COLS, self.rn.grid, self.rn.start, self.rn.end)
                try:
            ...
            cv2.putText(frame, f"ALGO: {self.algo_mode}", (8, 32),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 50, 0), 1, cv2.LINE_AA)
            ...
            if key == ord('q'):
                ...
                self.mouse_mode = "WALL"
            elif key == ord('1') and self.state == "EDIT":
                self.algo_mode = "BFS (Teammate)"
                print("Selected Algorithm: BFS (Teammate)")
            elif key == ord('2') and self.state == "EDIT":
                self.algo_mode = "BFS (User)"
                print("Selected Algorithm: BFS (User)")
            elif key == ord('3') and self.state == "EDIT":
                self.algo_mode = "DFS (User)"
                print("Selected Algorithm: DFS (User)")
            elif key == ord('r') and self.state == "DONE":
                prev_algo = self.algo_mode
                self.__init__()
                self.algo_mode = prev_algo
